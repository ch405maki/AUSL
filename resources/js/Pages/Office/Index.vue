<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Office</h2>
        </template>
                        ...
                        <tbody>
                            <tr v-for="(office, index) in offices" :key="office.id">
                                ...
                                <div class="flex justify-center items-center gap-1">
                                    ...
                                    <!-- update btn -->
                                    <button @click="deleteOffice(office.id, office.office_name)" class="p-2 rounded-full  group transition-all duration-500  flex item-center" title="Delete">
            ...
            <main class="flex-grow">
                <div class="max-w-7xl mx-auto">
                    <div class="flex flex-col md:flex-row w-full justify-center mb-6">
                        ...
                        <v-card-title class="flex justify-between items-center">
                            <div class="flex justify-end p-2">
                                <span class="headline">Add Office</span>
                            ...
                            <form @submit.prevent="submitForm" class="w-full">
                                <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-12">
                                    ...
                                    <div class="sm:col-span-6">
                                        ...
                                        <select v-model="form.category" id="category" class="mt-1 block w-full border border-gray-300 rounded-lg" required>
                                    ...
                                    <div class="sm:col-span-8">
                                        ...
                                        <!-- Error for Office Name -->
                                        <div v-if="form.errors.office_name" class="text-red-500 text-sm mt-1">{{ form.errors.office_name }}</div>
                                    </div>

                                    <!-- Office Location -->
                                    <div class="sm:col-span-12">
                                        <label for="office_location" class="block text-sm font-medium text-gray-700 mb-1">Office Location</label>
                                        <ckeditor :editor="editor" v-model="form.office_location" :config="editorConfig"></ckeditor>
                                        <!-- Error for Office Location -->
                                        <div v-if="form.errors.office_location" class="text-red-500 text-sm mt-1">{{ form.errors.office_location }}</div>
                                    </div>
                    ...
                    <v-card>
                        <v-card-title class="flex justify-between items-center">
                        <div class="flex justify-end p-2">
                            <span class="headline">Edit Office</span>
                        ...
                        <v-card-text>
                        <form @submit.prevent="updateOffice" class="w-full">
                            <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-12">
                            <!-- Category -->
                            <div class="sm:col-span-6">
                                <label for="edit_category" class="block text-sm font-medium text-gray-700">Category</label>
                                <select v-model="editForm.category" id="edit_category" class="mt-1 block w-full border border-gray-300 rounded-lg" required>
                                <option value="Department">Department</option>
                                <option value="Facilities">Facilities</option>
                                </select>
                            ...
                            <!-- Office Name -->
                            <div class="sm:col-span-8">
                                <label for="edit_office_name" class="block text-sm font-medium text-gray-700">Name</label>
                                <input type="text" id="edit_office_name" v-model="editForm.office_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required />
                                <div v-if="editForm.errors.office_name" class="text-red-500 text-sm mt-1">{{ editForm.errors.office_name }}</div>
                            </div>

                            <!-- Office Location -->
                            <div class="sm:col-span-12">
                                <label for="edit_office_location" class="block text-sm font-medium text-gray-700 mb-1">Office Location</label>
                                <ckeditor :editor="editor" v-model="editForm.office_location" :config="editorConfig"></ckeditor>
                                <div v-if="editForm.errors.office_location" class="text-red-500 text-sm mt-1">{{ editForm.errors.office_location }}</div>
                            </div>
                            </div>

                            <div class="sm:col-span-12 flex justify-center mt-6">
...
const openEditModal = (office) => {
    editForm.id = office.id;
    editForm.office_name = office.office_name;
    editForm.office_location = office.office_location;
    editForm.category = office.category;
    editPreviewImage.value = office.image;
...
const updateOffice = () => {
    ...
    formData.append('office_name', editForm.office_name);
    formData.append('office_location', editForm.office_location);
    formData.append('category', editForm.category);
    if (editForm.image) {
        formData.append('image', editForm.image);
    }

    editForm.put(route('offices.update', editForm.id), {
        data: formData,
        onSuccess: () => {
