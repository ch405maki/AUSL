<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Academic Guideline</h2>
        </template>
            ...
            <main class="flex-grow">
                ...
                <div class="flex flex-col md:flex-row w-full justify-center mb-6">
                    ...
                    <v-table fixed-header class="shadow-sm rounded-lg">
                        ...
                        <tbody>
                            <tr v-for="(guideline , index) in guidelines">
                                <td>{{ guideline.title }}</td>
                                <td v-html="guideline.content.substring(0, 300)"></td>
                                <td>
                                    <div class="flex justify-center items-center gap-1">
                                    <!-- update btn -->
                                    <button @click="deleteGuide(guideline.id, guideline.title)" class="p-2 rounded-full  group transition-all duration-500  flex item-center" title="Delete">
                            ...
                            <v-card>
                                <v-card-title class="flex justify-between items-center">
                                    <div class="flex justify-end p-2">
                                        <span class="headline">Add Academic Guideline</span>
                                    ...
                                    <form @submit.prevent="submitForm" class="w-full">
                                        <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-12">
                                            ...
                                            <div class="sm:col-span-8">
                                                <label for="office_name" class="block text-sm font-medium text-gray-700">Title</label>
                                                <input type="text" id="office_name" v-model="form.title" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required />
                                            </div>
                                            
                                            <!-- Office Location -->
                                            <div class="sm:col-span-12">
                                                <label for="office_function" class="block text-sm font-medium text-gray-700 mb-1">Body</label>
...
const submitForm = () => {
    ...
    form.post(route('academic_guidelines.store'), {
        data: formData,
        onSuccess: () => {
...
const deleteGuide = (id, title) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      form.delete(route('academic_guidelines.destroy', id));
    }
  });
};
