<template>
        ...
        <main class="flex-grow">
          <div class="max-w-7xl mx-auto">
            <div class="flex flex-col md:flex-row w-full justify-center mb-6">
                  ...
                  <tbody>
                    <tr v-for="(post, index) in carousells" :key="post.id">
                      ...
                      <td class="text-center">
                        ...
                        <button @click="reorderModal = true" class="p-2  rounded-full  group transition-all duration-500  flex item-center" title="Re-Order Items">
              ...
              <!-- Modal for Re order Carousell -->
              <v-dialog v-model="reorderModal" max-width="800px">
                <v-card>
                  <v-card-title class="flex justify-between items-center">
                    <div class="flex justify-end p-2">
                      <span class="headline">Re-Order Images</span>
                      <button @click="reorderModal = false" type="button"
                  ...
                  <v-card-text>
                    <form @submit.prevent="reorderForm" class="w-full">
                      <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-12">
                        ...
                        <div class="col-span-full">
                          <v-table fixed-header class="shadow-sm rounded-lg">
                            ...
                            <tbody>
                              <tr v-for="(post, index) in reorderData.carousells" :key="post.id">
                                <td><img :src="post.image" alt="Image Logo" class="carousell-image my-4"></td>
                                <td class="text-center">
                                  <input v-model.number="post.order" type="number" min="1" class="mt-1 block w-full border border-gray-300 rounded-lg"/>
                                </td>
                              </tr>
                            </tbody>
                          </v-table>
  ...
  carousells: props.carousells.map(carousell => ({
    id: carousell.id,
    order: carousell.order,
    image: carousell.image,
  })),
});

const reorderForm = () => {
  Inertia.patch(route('carousell.updateOrder'), reorderData.value, {
    onSuccess: () => {
      Swal.fire({
        title: 'Success!',
