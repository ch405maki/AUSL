<template>
  ...
  <AuthenticatedLayout>
      ...
      <template #action>
        <div class="flex justify-start flex-grow p-5">
                                  ...
                                  <tr v-for="document in documents" :key="document.id" >
                                      ...
                                      <td  class="text-center">
                                        <div class="flex justify-center items-center gap-1">
                                          <!-- update btn -->
                                          <button @click.prevent="downloadDocument(document.id)" class="p-2  rounded-full  group transition-all duration-500  flex item-center">
                                          ...
                                          <button @click="confirmDelete(document.id)" class="p-2 rounded-full  group transition-all duration-500  flex item-center">
...
const uploadDocument = async () => {
  ...
  try {
    await router.post('/documents', formData, {
      onSuccess: () => {
        Swal.fire({
          icon: 'success',
          title: 'Success!',
      ...
      onError: () => {
        Swal.fire({
          icon: 'error',
          title: 'Error!',
          text: 'Failed to upload document. Please check your input.',
          confirmButtonText: 'OK',
        });
      },
    });
...
const deleteDocument = async (documentId) => {
    try {
        await router.delete(`/documents/${documentId}`, {
            onSuccess: () => {
                Swal.fire({
                    icon: 'success',
                    title: 'Deleted!',
