<template>
  ...
  <AuthenticatedLayout>
      <template #header>
          <h2 class="font-semibold text-xl text-gray-800 leading-tight">On Load Banner</h2>
      </template>
              ...
              <div class="max-w-7xl mx-auto">
                  <div class="flex flex-col md:flex-row w-full justify-center">
                      ...
                      <div class="w-full md:w-2/3 p-1">
                        ...
                        <tbody>
                          <tr v-for="(onLoadBanner, index) in onLoadBanners" :key="onLoadBanner.id">
                            <td>{{ onLoadBanner.title }}</td>
                            <td>{{ onLoadBanner.url }}</td>
                            <td class="px-6 py-4">
                              <div class="flex justify-center">
                                <template v-if="onLoadBanner.state">
                            ...
                            <td>
                              <div class="flex items-center gap-1">
                                <button @click="openChangeStateModal(onLoadBanner)" class="p-2  rounded-full  group transition-all duration-500  flex item-center">
                                ...
                                <button @click="deleteBanner(onLoadBanner.id)" class="p-2 rounded-full  group transition-all duration-500  flex item-center">
                      ...
                      <div class="w-full md:w-1/3 mt-1 ml-1 flex justify-start">
                        <form @submit.prevent="submitForm" class="w-full bg-white p-5 rounded-lg">
                          <!-- Title -->
                          <div class="text-left">
                              <h2 class="text-xl font-bold">Onload Banner</h2>
                          </div>
                          <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-12">
                            ...
                            <!-- Confirm Button Text Input -->
                            <div class="sm:col-span-12">
                                <InputLabel for="confirmButtonText" value="Confirm Button Text" />
                                <TextInput
                                    id="confirmButtonText"
                                    v-model="form.confirmButtonText"
                                    autofocus
                                    required
                                    type="text"
                                    class="mt-1 block w-full border border-gray-300 rounded-lg "
                                    placeholder="Enter confirm button text..."
                                />
                            </div>

                            <!-- Cancel Button Text Input -->
                            <div class="sm:col-span-12">
                                <InputLabel for="cancelButtonText" value="Cancel Button Text" />
                                <TextInput
                                    id="cancelButtonText"
                                    v-model="form.cancelButtonText"
                                    autofocus
                                    required
                                    type="text"
                                    class="mt-1 block w-full border border-gray-300 rounded-lg "
                            ...
                            <!-- State Input -->
                            <div class="sm:col-span-12">
                              <InputLabel for="state" text="State" />
                              <select
                                v-model="form.state"
                                class="mt-1 block w-full border border-gray-300 rounded-lg"
                                id="state"
...
// Icon options
const iconOptions = ['info', 'success', 'error', 'warning', 'question'];

// Function to handle form submission
const submitForm = () => {
    form.post(route('onload-banners.store'), {
        onSuccess: () => {
...
const isChangeStateModalOpen = ref(false);
const selectedBanner = ref(null);
const newState = ref(null);

const openChangeStateModal = (banner) => {
  selectedBanner.value = banner;
  newState.value = banner.state;
  isChangeStateModalOpen.value = true;
};

const closeChangeStateModal = () => {
  isChangeStateModalOpen.value = false;
};

const changeState = async () => {
  if (selectedBanner.value) {
    try {
      await Inertia.put(route('onload-banner.update', selectedBanner.value.id), {
        state: newState.value
      });
      // Optionally, you may want to refresh the page or update the local state
...
const deleteBanner = (id) => {
    ...
    alerta.fire({
        ...
        if (result.isConfirmed) {
            form.delete(route('onload-banners.destroy', id));
        }
    });
};
</script>
